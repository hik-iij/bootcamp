...
## 演習0. 事前準備
dockerイメージとして <https://hub.docker.com/_/python/> および <https://hub.docker.com/_/redis> を用いています。
...
以下のような`compose.yaml`を作成し、FastAPI開発用、値の保存用(redis)2つのコンテナを起動してください。
...
- 動作確認
  ...
  18cdba0b1169   redis:alpine    "docker-entrypoint.s…"   21 seconds ago   Up 4 seconds   6379/tcp                                      iijbootcamp-backend
  ```
  - 
    - ```bash
      /app# [ $(docker compose ps | grep Up | wc -l) -eq 2 ] && echo "OK" >&2 || docker comopse ps
      ```
- docker コンテナの終了
  ```bash
  $ docker compose down
